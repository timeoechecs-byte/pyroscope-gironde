...
| **PHASE 5 (gate)** | **Inventaire terminé. Cible retenue = option A** (cellule-jour grille 1,5 km, FIRMS archive 2006-2024 ∪ Sarrau & Yagoub 2025 ∪ EFFIS polygones). Échantillonnage négatif **matched-pair spatio-temporel** (5 spatial, 5 temporel, 10 % uniforme de garde), validation hold-out **doublement bloquée** (forward-chaining temporel + GroupKFold 10×10 km spatial), tests de fuite automatiques (5 types). Déséquilibre attendu $10^{-4}$ à $10^{-5}$. AUC-PR + Brier + diagramme de fiabilité obligatoires ; AUC-ROC jamais principal ; exactitude jamais mise en avant. **Décision gate : PURSUE conditionnel** sous (a) vérification granularité Sarrau/Yagoub, (b) téléchargement effectif du dataset Gironde, (c) baseline FWI + coeff local **battu en validation spatiale** hors IC bootstrap. Document [`docs/PHASE5_FEASIBILITY.md`](PHASE5_FEASIBILITY.md). Trois replis successifs documentés : (B) reformulation commune-jour BDIFF, (C) surface parcourue conditionnelle, (D) abandon documenté ([`docs/PHASE5_OUTCOME.md`](PHASE5_OUTCOME.md)). | Sans (a), (b) ou (c) → PHASE5_OUTCOME.md rédigé sans entraînement. Sans battre le baseline FWI en validation spatiale → abandon même si AUC-PR élevée en validation aléatoire (signature classique de fuite spatio-temporelle). Jamais de probabilité ni de « confiance : X % » exposée à l'utilisateur final. |
| **PHASE 6** | **Mise en production, diffusion et résilience.** Performance : MVT pré-générés, Redis avec invalidation pilotée par l’ingestion (pas de TTL aveugle), Brotli, ETag, budget perf mesuré sur appareil modeste réel (premier rendu < 2 s en 4G). Test de charge à 50× nominal, point de rupture documenté. Observabilité Prometheus étendue (latence par quantile, 5xx, occupation disque/base, âge du dernier calcul). Alerting technique **distinct** des notifications utilisateur (cf. §6.2). Surveillance de zone utilisateur : notifications **jamais présentées comme alerte de sécurité**, repli e-mail/RSS obligatoire (push web inégalement fiable, en particulier iOS), anti-rebond, données personnelles minimales (pas de compte si jeton anonyme suffit). PWA installable + mode hors-ligne affichant l’âge de la donnée. Export GeoJSON/CSV/GeoPackage + API publique OpenAPI 3.1 **versionnée** + limitation de débit + politique de dépréciation. Posture réglementaire : différenciation explicite d’avec la vigilance officielle Météo-France/préfecture, **jamais de reprise des codes couleur ni du vocabulaire officiels**, mentions légales, RGPD, procédure de retrait rédigée **à froid**. **Mode crise** activable manuellement (bandeau prioritaire préfecture/SDIS 33 au-dessus de toute donnée, désactivation des fonctions coûteuses, fallback page statique légère). Critères détaillés : [`docs/PHASE_PLAN.md`](PHASE_PLAN.md) §PHASE 6. | Sans séparation alertes techniques / notifications utilisateur, le canal de crise opérationnelle se confond avec la supervision d’infra et les deux deviennent inutilisables. Sans posture réglementaire (différenciation d’avec la vigilance officielle), l’exposition au risque juridique et humain est injustifiable. |
| **PHASE 7** | **Pérennité, ouverture et extension.** Documentation scientifique : [`docs/METHODOLOGY.md`](METHODOLOGY.md) (chaîne complète + hypothèses non validées : table combustible, NDMI → humidité vivant, poids coefficient local) et [`docs/LIMITATIONS.md`](LIMITATIONS.md) (domaine de validité propagation, absence sautes de feu, latence sources). Notebooks de reproduction par résultat clé. Ouverture du code : choix de licence conscient (MIT / Apache 2.0 / AGPL), cohérence avec les contraintes non commerciales d’Open-Meteo documentée, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, CI publique, versionnement sémantique, `CHANGELOG.md`. **Revue externe** à trois regards : scientifique (INRAE), opérationnel (SDIS 33, DFCI), technique (dépôt ouvert) ; retour consigné dans [`docs/REVIEWS.md`](REVIEWS.md), y compris critiques non suivies avec raison. Extension géographique **conditionnée** à un socle solide : 33 → 33+40+47, grilles déjà paramétrables, coût calcul/stockage mesuré avant. **Plan de continuité** : arrêt propre (bascule en mode dégradé explicite au-delà d’un seuil de retard), bandeau « données non mises à jour depuis X jours », passation documentée. Décision tranchée : **projet personnel ou avec utilisateurs** — sans trancher, le second est choisi sans ses obligations. | Sans la décision « personnel / avec utilisateurs », le projet dérive lentement vers le second sans en assumer les contraintes (RGPD, continuité, support). Sans revue externe à trois regards, le moteur de risque reste non validé par quiconque d’extérieur au projet. |
| **Clôture** | **La feuille de route s'arrête à la phase 7. Il n'y a pas de phase 8.** Tout ce qui viendra après relève du **backlog** ([`docs/BACKLOG.md`](BACKLOG.md)). Le tableau « éléments volontairement retirés » (webcams, Blitzortung, LLM dans le risque, rafraîchissement 5 min, grille FWI 250 m, probabilité en pourcentage, horizons > 48 h) doit rester affiché en permanence — il définit le projet autant que ses livrables. | Empêche la réintroduction d’idées écartées pour de bonnes raisons. Empêche la renaissance d’une roadmap qui s’allonge plus vite qu’elle ne se vide. |
