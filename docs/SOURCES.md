...
| **Implémentation de référence CFFWIS / FBP — Python `cffdrs`** | Implémentée **PHASE 2 / PHASE 4** | aucune — dépendance python pure | package PyPI | 0 € | MIT (Canadian Forest Service) | **Implémentation canonique** du CFFWIS et du FBP, maintenue par le Service canadien des forêts. Utilisée comme référence pour valider les sorties des modules `cffwis.py` (PHASE 2) et `fbp.py` + `rothermel.py` (PHASE 4). Tests externes : `inst/extdata/test_fwi.csv` + `cffdrs::test_fbp_*`. **Ne pas inventer les constantes** : chaque paramètre de notre réimplémentation renvoie au numéro d'équation ou au tableau de la publication. |
| **Implémentation de référence Rothermel — `pyrolog`** | Implémentée **PHASE 4** | aucune — dépendance python pure | package PyPI | 0 € | vérifier license du fork | Implémentation Python open source de Rothermel 1972 + Scott & Burgan 40, à valider contre la sortie de notre `rothermel.py` (PHASE 4 Scope B). Référence canonique historique : Andrews (2018) RMRS-GTR-371, BehavePlus. |
